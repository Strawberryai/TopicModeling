...
def main():
    ...
    lgr = LogisticRegression(penalty="l2" ,C=2, n_jobs=-1, random_state=1337)
    gnb = GaussianNB()
    mnb = MultinomialNB()
    bnb = BernoulliNB()

    lgr.class_weight = "balanced"
    gnb.class_weight = "balanced"
    mnb.class_weight = "balanced"
    bnb.class_weight = "balanced"

    lgr.fit(trainX, trainY)
    y_pred_lgr = lgr.predict(devX)

    gnb.fit(trainX, trainY)
    y_pred_gnb = gnb.predict(devX)

    mnb.fit(trainX, trainY)
    y_pred_mnb = mnb.predict(devX)

    bnb.fit(trainX, trainY)
    y_pred_bnb = bnb.predict(devX)

    reportLGR = classification_report(devY,y_pred_lgr, output_dict=True)
    reportGNB = classification_report(devY,y_pred_gnb, output_dict=True)
    reportMNB = classification_report(devY,y_pred_mnb, output_dict=True)
    reportBNB = classification_report(devY,y_pred_bnb, output_dict=True)

    print("\nLGR REPORT")
    print(pd.DataFrame(reportLGR).transpose())

    print("\nGNB REPORT")
    print(pd.DataFrame(reportGNB).transpose())

    print("\nMNB REPORT")
    print(pd.DataFrame(reportMNB).transpose())

    print("\nBNB REPORT")
    print(pd.DataFrame(reportBNB).transpose())

    guardar_modelo(lgr, "LGR")
    guardar_modelo(gnb, "GNB")
    guardar_modelo(mnb, "MNB")
    guardar_modelo(bnb, "BNB")
